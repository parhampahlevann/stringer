#!/bin/bash
# ============================================================================
# Stinger Tunnel - Fixed Edition v12 (With Debugging & MTU Fix)
# ============================================================================
...
TUN_MTU=1280  # FIX: Reduced MTU to 1280 to prevent TCP packet drop in ICMP
...
forward_ports_to_foreign() {
    ...
    if [ ! -f "${INSTALL_DIR}/config.toml" ]; then
        print_error "Config not found. Install Stinger first."
        return 1
    fi
...
# NEW FEATURE: Debug with tcpdump
debug_traffic() {
    ...
    read -p "  Enter port to trace (e.g. 80): " PORT </dev/tty
    TUN_IFACE=$(ip -o link show 2>/dev/null | awk -F': ' '/tun|flagtun/ {print $2; exit}')
    if [ -z "$TUN_IFACE" ]; then
        print_error "TUN interface not found. Is Stinger running?"
        return 1
    fi
    print_info "=================================================="
    print_info "Listening on $TUN_IFACE for port $PORT..."
    print_info "Try connecting to this server's Public IP on port $PORT"
    print_info "Press Ctrl+C to stop."
    print_info "=================================================="
    tcpdump -n -i "$TUN_IFACE" "port $PORT"
    read -p "Press Enter..." </dev/tty
}
...
while true; do
    clear
    echo "============================================"
    echo "  Stinger Tunnel - Fixed Edition v12"
    echo "============================================"
    echo ""
